...
func IAMUserAccounts() *schema.Table {
	return &schema.Table{
		Name:        "yandex_iam_user_accounts",
		Resolver:    fetchIAMUserAccounts,
...
func fetchIAMUserAccounts(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan interface{}) error {
	...
	g.Go(func() error {
		defer close(ch)
		return fetchAccessBindings(ctx, meta, parent, ch)
	})

	g.Go(func() error {
		for value := range ch {
			id := value.(RichAccessBinding).AccessBinding.Subject.Id
			accountType := value.(RichAccessBinding).AccessBinding.Subject.Type
